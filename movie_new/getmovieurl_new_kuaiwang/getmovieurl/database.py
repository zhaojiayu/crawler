...
class CreateDB():
    ...
    def create_table(self):
        ...
        self.cur.execute("create table new_movies( movie_name varchar(100), movie_rate varchar(10), \
...
class DBOperation():
    ...
    def update_data(self, value):
        try:
            sql = "UPDATE new_movies SET movie_url = '%s',movie_picurl = '%s' WHERE movie_name = '%s'" % (value['movie_url'], value['movie_picurl'], value['movie_name'])
    ...
    def select_data(self):
        try:
            max_sql = "SELECT max(movie_id) FROM new_movies WHERE movie_url !=''"
            self.cur.execute(max_sql)
            number = self.cur.fetchall()
            print number
            if number[0][0]:
                print 'not none'
                sql = "SELECT movie_name FROM new_movies WHERE movie_id > %s" % number[0][0]
            else:
                print 'none'
                sql = "SELECT movie_name FROM new_movies"
        #    print sql
            self.cur.execute(sql)
    ...
    def select_col(self):
        try:
            sel_col_sql = "SELECT movie_name,movie_url FROM new_movies"
    ...
    def delete_col(self, value):
        try:
            del_col_sql = "DELETE FROM new_movies WHERE movie_name = '%s'" % value['movie_name']
